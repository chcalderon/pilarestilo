...
import com.pilarestilo.productai.infrastructure.ai.ProductAiOpenAiClient;
...
public class ProductAiService {
    ...
    public ProductAiService(ProductAiDraftJpaRepository draftRepository,
                            ...
                            ProductAiOpenAiClient openAiClient) {
    ...
    public ProductAiInferenceDto inferSingleImage(MultipartFile file, String brandHint) {
        ...
        try {
            ProductAiOpenAiClient.InferenceResult inference = openAiClient.inferFromImage(
                    file.getBytes(),
    ...
    public ProductAiImageTransformDto transformSingleImage(
        ...
        try {
            ...
            byte[] transformedBytes = openAiClient.transformImage(
                    file.getBytes(),
                    file.getOriginalFilename(),
                    prompt
            );
    ...
    private void processSingleJob(ProductAiJobEntity job) {
        ...
        try {
            ...
            if ("stub".equalsIgnoreCase(productAiEngine)) {
                processViaStub(job, draft, assets);
            } else {
                processViaBackendOpenAi(job, draft, assets);
            }
    ...
    private void processViaBackendOpenAi(ProductAiJobEntity job, ProductAiDraftEntity draft, List<ProductAiAssetEntity> assets) {
        int total = assets.size();
        int index = 0;
        for (ProductAiAssetEntity asset : assets) {
            byte[] sourceBytes = nodeBridgeClient.loadOriginalAssetBytes(asset.getOriginalUrl());
            ProductAiOpenAiClient.InferenceResult inferred = openAiClient.inferFromImage(
                    sourceBytes,
            ...
            String prefix = "job-" + shortId(job.getId()) + "-" + shortId(asset.getId());
            ProductAiNodeBridgeClient.StoredDerivatives derivatives =
                    nodeBridgeClient.storeDerivatives(folder, prefix, transformedBytes);
            ...
            asset.setProcessedMasterUrl(derivatives.masterUrl());
            asset.setProcessedWebUrl(derivatives.webUrl());
            asset.setProcessedThumbUrl(derivatives.thumbUrl());
            assetRepository.save(asset);
    ...
    private String normalizeTransformProvider(String providerRaw) {
        ...
        if (!"openai".equals(normalized)) {
            throw new DomainException("Proveedor IA no soportado: " + providerRaw + ". Usa OPENAI.");
        }
        return normalized;
    }
