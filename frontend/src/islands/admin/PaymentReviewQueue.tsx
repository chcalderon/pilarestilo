...
export default function PaymentReviewQueue() {
  ...
  async function handleAction(id: string, action: 'approve' | 'reject') {
    ...
    if (!reviewerId) {
      alert('No se pudo identificar al administrador actual. Vuelve a iniciar sesion.');
      return;
    }
  ...
  const columns: Column<PaymentDto>[] = [
    {
      ...
      header: 'ID Orden',
      render: (row) => (
        <span className="font-mono text-[0.72rem] text-pe-charcoal/50">{String(row.orderId).slice(0, 8)}...</span>
      ),
    },
    {
      key: 'method',
