...
export default function DataTable<T>({
  ...
  function toggleAll() {
    if (selected.size === data.length) {
      ...
      setSelected(new Set(data.map(r => String((r as Record<string | symbol, unknown>)[keyField as symbol]))));
    }
  }
  ...
  function SortIcon({ colKey }: { colKey: string }) {
    ...
    return sortDir === 'asc'
      ? <ChevronUp size={12} className="text-pe-rose" />
      : <ChevronDown size={12} className="text-pe-rose" />;
  }

  const thBase = 'font-sans text-[0.68rem] tracking-[0.1em] uppercase text-pe-charcoal/50 px-3 py-2.5 text-left whitespace-nowrap border-b border-pe-black/8 bg-pe-cream/60';
  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      {selectable && selected.size > 0 && bulkActions && (
        <div className="flex items-center gap-3 px-4 py-2.5 bg-pe-rose/8 border border-pe-rose/20 mb-2">
          ...
          <div className="flex gap-2 ml-2">
            {bulkActions.map((action, i) => (
              <button
                key={i}
                onClick={() => { action.action(Array.from(selected)); setSelected(new Set()); }}
                className={[
          ...
          <button
            ...
            className="ml-auto font-sans text-[0.72rem] text-pe-charcoal/40 hover:text-pe-charcoal transition-colors"
          >
            Cancelar
          </button>
        </div>
      ...
      {/* Table */}
      <div className="overflow-x-auto bg-pe-white border border-pe-black/6 shadow-sm">
        <table className="w-full min-w-max" role="grid">
          ...
          <tbody>
            {loading ? (
              ...
              data.map(row => {
                const id = String((row as Record<string | symbol, unknown>)[keyField as symbol]);
                ...
                return (
                  ...
                  >
                    {selectable && (
                      <td className={tdBase} onClick={e => { e.stopPropagation(); toggleRow(id); }}>
                        <input
                          type="checkbox"
                          checked={isSelected}
